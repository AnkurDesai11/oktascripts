...
input_user_list = pandas.read_csv(input_file_path, header=0, keep_default_na=False).loc[:,['uid','sms','voice']]
...
def worker_thread():
    ...
    while not shared_queue.empty():
        try:
            ...
            sms_factor = current_user_details["sms"]
            voice_factor = current_user_details["voice"]
            ...
            if response.status_code==200:
                if(len(response.json()) != 0):
                    ...
                    current_user_details.at[0, 'MFA_SMS_Number'] = sms_factor
                    current_user_details.at[0, 'MFA_Voice_Number'] = voice_factor
                    endpoint1 = "{}/api/v1/users/{}/factors?activate=true".format(base_url,current_user_details["id"].values[0])
                    body1 = { "factorType": "sms", "provider": "OKTA", "profile": { "phoneNumber": '"'+str(current_user_details["MFA_SMS_Number"].values[0])+'"' } }
                    response1 = requests.post(endpoint1, data=json.dumps(body1), headers=headers, verify = False)
                    if response1.status_code==200:
                        current_user_details.at[0, 'MFA_SMS_Status'] = response1.json()["status"]
                    else:
                        current_user_details.at[0, 'MFA_SMS_Status'] = response1.json()["errorCauses"][0]["errorSummary"] if "errorSummary" in response1.json()["errorCauses"][0] else response1.json()["errorSummary"]
                    
                    endpoint2 = "{}/api/v1/users/{}/factors?activate=true".format(base_url,current_user_details["id"].values[0])
                    body2 = { "factorType": "call", "provider": "OKTA", "profile": { "phoneNumber": '"'+str(current_user_details["MFA_Voice_Number"].values[0])+'"' } }
                    response2 = requests.post(endpoint2, data=json.dumps(body2), headers=headers, verify = False)
                    if response2.status_code==200:
                        current_user_details.at[0, 'MFA_Voice_Status'] = response2.json()["status"]
                    else:
                        current_user_details.at[0, 'MFA_Voice_Status'] = response2.json()["errorCauses"][0]["errorSummary"] if "errorSummary" in response1.json()["errorCauses"][0] else response1.json()["errorSummary"]
