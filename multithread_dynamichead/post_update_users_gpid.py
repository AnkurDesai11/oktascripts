...
selected_columns[len(selected_columns):len(selected_columns)] = ["Update_Status"]
...
if not id_to_continue_from:
    ...
    if output_file_path == "user_updates_"+append_time+".csv":
        batch_list.to_csv(output_file_path, index=False)
        ...
        try:
            ...
            if response.status_code==200:
                if(len(response.json()) != 0):
                    ...
                    current_user_details_recieved.at[0, 'result'] = "Found"
                    endpoint1 = "{}/api/v1/users/{}".format(base_url,current_user_details_recieved["id"].values[0])
                    body = {}
                    for  value in values_to_update:
                        ...
                        try:
                            ...
                            current_user_details_recieved.at[0, 'result'] = "Found - error in request body while updating"
                    ...
                    response1 = requests.post(endpoint1, data=json.dumps(body), headers=headers, verify = False)
                    if response1.status_code==200:
                        current_user_details_recieved.at[0, 'Update_Status'] = "success"
                    else:
