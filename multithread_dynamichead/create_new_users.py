...
################################################################
#selected_columns[len(selected_columns):len(selected_columns)] = [ "MFA_SMS_Number", "MFA_SMS_Status", "MFA_Voice_Number", "MFA_Voice_Status"]
...
output_file_path = input("OPTIONAL - Enter filepath to save output (absolute); DEFAULT - 'new_users_[timestamp].csv' in the same location as script: ") or "new_users_"+append_time+".csv"
...
input_user_list = pandas.read_csv(input_file_path, header=0, keep_default_na=False)
values_to_update = input_user_list.columns.values[1:]
selected_columns[len(selected_columns):len(selected_columns)] = values_to_update
selected_columns[len(selected_columns):len(selected_columns)] = ["Create_Status"]
...
if not id_to_continue_from:
    ...
    if id_to_continue_from in input_user_list['uid'].values:
        input_user_list = input_user_list[input_user_list[input_user_list['uid'] == id_to_continue_from].index[0]+1:]
    else:
        print(id_to_continue_from, "not found, appending current output to", output_file_path)
    if output_file_path == "new_users_"+append_time+".csv":
        batch_list.to_csv(output_file_path, index=False)
...
[shared_queue.put(row) for row in input_user_list.to_dict('records')]
...
def get_pair(nested_key, value):
    if len(nested_key) == 1:
        return {nested_key[0]: value}
    else:
        return {nested_key[0]: get_pair(nested_key[1:], value)}

def update_body(body, to_add):
    if list(to_add.keys())[0] in body.keys():
        body.update({list(to_add.keys())[0]:update_body(body[list(to_add.keys())[0]], to_add[list(to_add.keys())[0]])})
        return body
    else:
        body.update(to_add)
        return body
...
def worker_thread():
    ...
    while not shared_queue.empty():
        try:
            current_user_details = shared_queue.get()
            ...
            #current_uid = uid
            #uid="\""+uid+"\""
            current_user_details_received = pandas.DataFrame(columns = selected_columns)
            body = {}
            for  value in values_to_update:
                current_key_value = get_pair(value.split('.'), current_user_details[value])
                try:
                    body = update_body(body, current_key_value)
                except Exception as e:
                    current_user_details_recieved.at[0, 'result'] = "Error in forming request body while creating"
                    logging.warning('Error while adding key value: %s %s', current_key_value, e)
                    print("Error while adding key value", current_key_value, " to body object: ",e)
            endpoint = "{}/api/v1/users?activate=true".format(base_url)
            ...
            if response.status_code==200:
                if(len(response.json()) != 0):
                    current_user_details_recieved = pandas.json_normalize(response.json(), errors='ignore').reindex(columns=selected_columns).fillna('')
                    current_user_details_recieved.at[0, 'uid'] = current_uid
                    current_user_details_recieved.at[0, 'result'] = "success" if current_user_details_recieved.at[0, 'result'] != None and current_user_details_recieved.at[0, 'result'] != "" else current_user_details_recieved.at[0, 'result']
                    ...
                    current_user_details.at[0, 'result'] = "Failed to create user with 200 OK response"
                    batch_list = pandas.concat([batch_list, current_user_details], ignore_index=True)
            else:
                current_user_details = pandas.DataFrame(columns = selected_columns)
                current_user_details.at[0, 'uid'] = current_uid
                current_user_details.at[0, 'result'] = "Failed to create user - "+response.status_code
                batch_list = pandas.concat([batch_list, current_user_details], ignore_index=True)
