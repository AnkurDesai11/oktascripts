        ...
        try:
            ...
            if response.status_code==200:
                if(len(response.json()) != 0):
                    ...
                    body1 = { "factorType": "sms", "provider": "OKTA", "profile": { "phoneNumber": current_user_details["MFA_SMS_Number"].values[0] } }
                    response1 = requests.post(endpoint1, data=body1, headers=headers, verify = False)
                    if response1.status_code==200:
                        current_user_details.at[0, 'MFA_SMS_Status'] = response1.json()["status"]
                    else:
                        current_user_details.at[0, 'MFA_SMS_Status'] = response1.json()["errorCauses"]["errorSummary"] if "errorSummary" in response1.json()["errorCauses"] else response1.json()["errorSummary"]
                    
                    endpoint2 = "{}/api/v1/users/{}/factors?activate=true".format(base_url,current_user_details["id"].values[0])
                    body2 = { "factorType": "call", "provider": "OKTA", "profile": { "phoneNumber": current_user_details["MFA_Voice_Number"].values[0] } }
                    response2 = requests.post(endpoint2, headers=headers, verify = False)
                    if response1.status_code==200:
                        current_user_details.at[0, 'MFA_SMS_Status'] = response1.json()["status"]
                    else:
                        current_user_details.at[0, 'MFA_SMS_Status'] = response1.json()["errorCauses"]["errorSummary"] if "errorSummary" in response1.json()["errorCauses"] else response1.json()["errorSummary"]
